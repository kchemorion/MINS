...
class qSlicerDICOMPatcherModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'dicompatcher'))
    self.module = slicer.modules.dicompatcher

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicerDICOMPatcherModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicerDICOMPatcherModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'DICOMPatcher')
