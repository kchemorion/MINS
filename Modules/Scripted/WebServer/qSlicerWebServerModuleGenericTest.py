...
class qSlicerWebServerModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'webserver'))
    self.module = slicer.modules.webserver

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicerWebServerModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicerWebServerModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'WebServer')
