...
class qSlicerScreenCaptureModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'screencapture'))
    self.module = slicer.modules.screencapture

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicerScreenCaptureModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicerScreenCaptureModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'ScreenCapture')
