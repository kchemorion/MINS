...
class qSlicerSegmentStatisticsModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'segmentstatistics'))
    self.module = slicer.modules.segmentstatistics

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicerSegmentStatisticsModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicerSegmentStatisticsModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'SegmentStatistics')
