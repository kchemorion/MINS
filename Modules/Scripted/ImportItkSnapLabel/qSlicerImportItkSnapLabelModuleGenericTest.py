...
class qSlicerImportItkSnapLabelModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'importitksnaplabel'))
    self.module = slicer.modules.importitksnaplabel

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicerImportItkSnapLabelModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicerImportItkSnapLabelModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'ImportItkSnapLabel')
