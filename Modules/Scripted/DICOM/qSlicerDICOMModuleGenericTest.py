...
class qSlicerDICOMModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'dicom'))
    self.module = slicer.modules.dicom

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicerDICOMModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicerDICOMModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'DICOM')
