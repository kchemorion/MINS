...
//-----------------------------------------------------------------------------
int qSlicerEventBrokerModuleWidgetGenericTest( int argc, char * argv[] )
{
  bool res = true;
  qSlicerApplication app(argc, argv);
  app.errorLogModel()->setTerminalOutputs(ctkErrorLogTerminalOutput::All);

  qSlicerEventBrokerModule module(0);
  ...
  if (widget->minimumSizeHint().width() > maxMinimumSizeHintWidth)
    ...
    std::cerr << "Line " << __LINE__
              << " qSlicerEventBrokerModule widget has a minimum size hint width"
