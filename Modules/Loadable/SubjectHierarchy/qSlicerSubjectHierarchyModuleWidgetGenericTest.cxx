...
//-----------------------------------------------------------------------------
int qSlicerSubjectHierarchyModuleWidgetGenericTest( int argc, char * argv[] )
{
  bool res = true;
  qSlicerApplication app(argc, argv);
  app.errorLogModel()->setTerminalOutputs(ctkErrorLogTerminalOutput::All);

  qSlicerSubjectHierarchyModule module(0);
  ...
  if (widget->minimumSizeHint().width() > maxMinimumSizeHintWidth)
    ...
    std::cerr << "Line " << __LINE__
              << " qSlicerSubjectHierarchyModule widget has a minimum size hint width"
