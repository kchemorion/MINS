...
//-----------------------------------------------------------------------------
int qSlicerSubjectHierarchyModuleGenericTest( int argc, char * argv[] )
{
  qSlicerApplication app(argc, argv);
  app.errorLogModel()->setTerminalOutputs(ctkErrorLogTerminalOutput::All);

  qSlicerSubjectHierarchyModule * module = new qSlicerSubjectHierarchyModule;
  module->setName("SubjectHierarchy");
  module->setObjectName(QString("%1Module").arg("SubjectHierarchy"));
  ...
  QString moduleFileName = "libqSlicerSubjectHierarchyModule.so";
  ...
  }
  {
    bool current = qSlicerLoadableModule::addModuleToSlicerModules(
          pythonManager, module, "SubjectHierarchy");
    bool expected = true;
    if (current != expected)
      {
  ...
  }
  {
    bool current = qSlicerLoadableModule::addModuleNameToSlicerModuleNames(
          pythonManager, "SubjectHierarchy");
    bool expected = true;
    if (current != expected)
      {
  ...
  if (module->title().isEmpty())
    ...
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::title() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->categories().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::categories() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->index() < -1 || module->index() > 1000)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::index() seems invalid." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  //  {
  //  std::cerr << "Line " << __LINE__
  //            << " - qSlicerSubjectHierarchyModule::helpText() is empty." << std::endl;
  //  return EXIT_FAILURE;
  //  }
  ...
  if (module->acknowledgementText().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::acknowledgementText() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->contributors().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::contributors() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->appLogic() != 0)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::initialize() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (!moduleRepresentation && !module->isHidden())
    ...
    std::cerr << "Line " << __LINE__
              << " qSlicerSubjectHierarchyModule has no widget representation."
  ...
  if (module->mrmlScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::setMRMLScene() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (logic && logic->GetMRMLScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerSubjectHierarchyModule::setMRMLScene() failed to pass"
