...
//-----------------------------------------------------------------------------
int qSlicerTerminologiesModuleGenericTest( int argc, char * argv[] )
{
  qSlicerApplication app(argc, argv);
  app.errorLogModel()->setTerminalOutputs(ctkErrorLogTerminalOutput::All);

  qSlicerTerminologiesModule * module = new qSlicerTerminologiesModule;
  module->setName("Terminologies");
  module->setObjectName(QString("%1Module").arg("Terminologies"));
  ...
  QString moduleFileName = "libqSlicerTerminologiesModule.so";
  ...
  }
  {
    bool current = qSlicerLoadableModule::addModuleToSlicerModules(
          pythonManager, module, "Terminologies");
    bool expected = true;
    if (current != expected)
      {
  ...
  if (module->title().isEmpty())
    ...
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::title() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->categories().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::categories() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->index() < -1 || module->index() > 1000)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::index() seems invalid." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  //  {
  //  std::cerr << "Line " << __LINE__
  //            << " - qSlicerTerminologiesModule::helpText() is empty." << std::endl;
  //  return EXIT_FAILURE;
  //  }
  ...
  if (module->acknowledgementText().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::acknowledgementText() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->contributors().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::contributors() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->appLogic() != 0)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::initialize() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (!moduleRepresentation && !module->isHidden())
    ...
    std::cerr << "Line " << __LINE__
              << " qSlicerTerminologiesModule has no widget representation."
  ...
  if (module->mrmlScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::setMRMLScene() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (logic && logic->GetMRMLScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerTerminologiesModule::setMRMLScene() failed to pass"
