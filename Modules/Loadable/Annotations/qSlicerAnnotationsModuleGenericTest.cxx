...
//-----------------------------------------------------------------------------
int qSlicerAnnotationsModuleGenericTest( int argc, char * argv[] )
{
  qSlicerApplication app(argc, argv);
  app.errorLogModel()->setTerminalOutputs(ctkErrorLogTerminalOutput::All);

  qSlicerAnnotationsModule * module = new qSlicerAnnotationsModule;
  module->setName("Annotations");
  module->setObjectName(QString("%1Module").arg("Annotations"));
  ...
  QString moduleFileName = "libqSlicerAnnotationsModule.so";
  ...
  }
  {
    bool current = qSlicerLoadableModule::addModuleToSlicerModules(
          pythonManager, module, "Annotations");
    bool expected = true;
    if (current != expected)
      {
  ...
  if (module->title().isEmpty())
    ...
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::title() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->categories().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::categories() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->index() < -1 || module->index() > 1000)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::index() seems invalid." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  //  {
  //  std::cerr << "Line " << __LINE__
  //            << " - qSlicerAnnotationsModule::helpText() is empty." << std::endl;
  //  return EXIT_FAILURE;
  //  }
  ...
  if (module->acknowledgementText().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::acknowledgementText() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->contributors().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::contributors() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->appLogic() != 0)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::initialize() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (!moduleRepresentation && !module->isHidden())
    ...
    std::cerr << "Line " << __LINE__
              << " qSlicerAnnotationsModule has no widget representation."
  ...
  if (module->mrmlScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::setMRMLScene() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (logic && logic->GetMRMLScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerAnnotationsModule::setMRMLScene() failed to pass"
