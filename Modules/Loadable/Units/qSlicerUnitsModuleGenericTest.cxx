...
//-----------------------------------------------------------------------------
int qSlicerUnitsModuleGenericTest( int argc, char * argv[] )
{
  qSlicerApplication app(argc, argv);
  app.errorLogModel()->setTerminalOutputs(ctkErrorLogTerminalOutput::All);

  qSlicerUnitsModule * module = new qSlicerUnitsModule;
  module->setName("Units");
  module->setObjectName(QString("%1Module").arg("Units"));
  ...
  QString moduleFileName = "libqSlicerUnitsModule.so";
  ...
  if (module->title().isEmpty())
    ...
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::title() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->categories().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::categories() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->index() < -1 || module->index() > 1000)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::index() seems invalid." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  //  {
  //  std::cerr << "Line " << __LINE__
  //            << " - qSlicerUnitsModule::helpText() is empty." << std::endl;
  //  return EXIT_FAILURE;
  //  }
  ...
  if (module->acknowledgementText().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::acknowledgementText() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->contributors().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::contributors() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->appLogic() != 0)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::initialize() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (!moduleRepresentation && !module->isHidden())
    ...
    std::cerr << "Line " << __LINE__
              << " qSlicerUnitsModule has no widget representation."
  ...
  if (module->mrmlScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::setMRMLScene() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (logic && logic->GetMRMLScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerUnitsModule::setMRMLScene() failed to pass"
